...
int main(int argc, char** argv) {
  ...
  cuteCheck(cuInit(0));
  int numberOfDevices;
  auto status = cuDeviceGetCount(&numberOfDevices);
  if (CUDA_SUCCESS != status) {
    std::cout << "Failed to initialize the CUDA runtime";
    return EXIT_FAILURE;
  }
  std::cout << "Found " << numberOfDevices << " devices" << std::endl;
  std::vector<CUcontext> context(numberOfDevices);
  for (int i = 0; i < numberOfDevices; ++i) {
    cuteCheck(cuCtxCreate(&context[i], CU_CTX_SCHED_YIELD | CU_CTX_MAP_HOST | CU_CTX_LMEM_RESIZE_TO_MAX, i));
    cuteCheck(cuDevicePrimaryCtxRetain(&context[i], i));
  }
  ...
  for (int i = 0; i < numberOfDevices; ++i) {
    cuteCheck(cuDevicePrimaryCtxRelease(i));
  }
