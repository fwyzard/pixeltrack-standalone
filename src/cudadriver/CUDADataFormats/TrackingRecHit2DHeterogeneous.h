...
public:
  ...
  cms::cuda::host::unique_ptr<float[]> localCoordToHostAsync(CUstream stream) const;
  cms::cuda::host::unique_ptr<uint16_t[]> detIndexToHostAsync(CUstream stream) const;
  cms::cuda::host::unique_ptr<uint32_t[]> hitsModuleStartToHostAsync(CUstream stream) const;

  // for validation
  cms::cuda::host::unique_ptr<float[]> globalCoordToHostAsync(CUstream stream) const;
  cms::cuda::host::unique_ptr<int32_t[]> chargeToHostAsync(CUstream stream) const;
  cms::cuda::host::unique_ptr<int16_t[]> sizeToHostAsync(CUstream stream) const;
