...
public:
  ...
  explicit SiPixelDigiErrorsCUDA(size_t maxFedWords, PixelFormatterErrors errors, CUstream stream);
  ~SiPixelDigiErrorsCUDA() = default;
  ...
  HostDataError dataErrorToHostAsync(CUstream stream) const;

  void copyErrorToHostAsync(CUstream stream);
