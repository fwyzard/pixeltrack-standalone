...
namespace cms {
  namespace cuda {
    ...
    public:
      ...
      // CUstream is a pointer to a thread-safe object, for which a
      // mutable access is needed even if the cms::cuda::ScopedContext itself
      // would be const. Therefore it is ok to return a non-const
      // pointer from a const method here.
      CUstream stream() const { return stream_.get(); }

      // CUevent is a pointer to a thread-safe object, for which a
      // mutable access is needed even if the cms::cuda::ScopedContext itself
      // would be const. Therefore it is ok to return a non-const
      // pointer from a const method here.
      CUevent event() const { return event_.get(); }
    ...
    private:
      ...
      // The CUstream is really shared among edm::Event products, so
