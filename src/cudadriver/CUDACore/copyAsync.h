...
namespace cms {
  namespace cuda {
    ...
    template <typename T>
    inline void copyAsync(device::unique_ptr<T>& dst, const host::unique_ptr<T>& src, CUstream stream) {
      ...
      cuteCheck(cuMemcpyAsync((CUdeviceptr)dst.get(), (CUdeviceptr)src.get(), sizeof(T), stream));
    }

    template <typename T>
    inline void copyAsync(device::unique_ptr<T>& dst, const host::noncached::unique_ptr<T>& src, CUstream stream) {
      // Shouldn't compile for array types because of sizeof(T), but
      // let's add an assert with a more helpful message
      static_assert(std::is_array<T>::value == false,
                    "For array types, use the other overload with the size parameter");
    ...
    template <typename T>
    inline void copyAsync(host::unique_ptr<T>& dst, const device::unique_ptr<T>& src, CUstream stream) {
      static_assert(std::is_array<T>::value == false,
                    "For array types, use the other overload with the size parameter");
      cuteCheck(cuMemcpyAsync((CUdeviceptr)dst.get(), (CUdeviceptr)src.get(), sizeof(T), stream));
    }
    ...
    inline void copyAsync(device::unique_ptr<T[]>& dst,
                          ...
                          size_t nelements,
                          CUstream stream) {
      cuteCheck(cuMemcpyAsync((CUdeviceptr)dst.get(), (CUdeviceptr)src.get(), nelements * sizeof(T), stream));
    }
