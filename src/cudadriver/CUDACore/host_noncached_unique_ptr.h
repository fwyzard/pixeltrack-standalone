...
namespace cms {
    ...
    namespace host {
      namespace noncached {
        namespace impl {
          ...
          class HostDeleter {
          public:
            void operator()(void *ptr) { cuteCheck(cuMemFreeHost(ptr)); }
          };
        }  // namespace impl
   ...
   */
    ...
    typename host::noncached::impl::make_host_unique_selector<T>::non_array make_host_noncached_unique(
      ...
      void *mem;
      cuteCheck(cuMemHostAlloc(&mem, sizeof(T), flags));
    ...
    typename host::noncached::impl::make_host_unique_selector<T>::unbounded_array make_host_noncached_unique(
        size_t n, unsigned int flags = 0) {
      using element_type = typename std::remove_extent<T>::type;
      static_assert(std::is_trivially_constructible<element_type>::value,
                    "Allocating with non-trivial constructor on the pinned host memory is not supported");
      void *mem;
      cuteCheck(cuMemHostAlloc(&mem, n * sizeof(element_type), flags));
