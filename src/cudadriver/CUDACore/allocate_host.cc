...
namespace cms::cuda {
  void *allocate_host(size_t nbytes, CUstream stream) {
    ...
    if constexpr (allocator::useCaching) {
      ...
      cuteCheck(allocator::getCachingHostAllocator().HostAllocate(&ptr, nbytes, stream));
    } else {
      cuteCheck(cuMemAllocHost(&ptr, nbytes));
    }
    return ptr;
  }
  ...
  void free_host(void *ptr) {
    if constexpr (allocator::useCaching) {
      cuteCheck(allocator::getCachingHostAllocator().HostFree(ptr));
    } else {
      cuteCheck(cuMemFreeHost(ptr));
    }
  }
