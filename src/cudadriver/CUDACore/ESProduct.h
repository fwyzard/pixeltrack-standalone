...
namespace cms {
  namespace cuda {
    ...
    public:
      ...
      // transferAsync should be a function of (T&, CUstream)
      // which enqueues asynchronous transfers (possibly kernels as well)
      // to the CUDA stream
      template <typename F>
      const T& dataForCurrentDeviceAsync(CUstream stream, F transferAsync) const {
        ...
        if (not data.m_filled.load()) {
          ...
          if (data.m_fillingStream != nullptr) {
            ...
            if (eventWorkHasCompleted(data.m_event.get())) {
              ...
              cuteCheck(cuStreamWaitEvent(stream, data.m_event.get(), 0),
    ...
    private:
      struct Item {
        ...
        // non-null if some thread is already filling (CUstream is just a pointer)
        mutable CUstream m_fillingStream = nullptr;  // guarded by m_mutex
