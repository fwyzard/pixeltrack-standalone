...
namespace cms {
  namespace cuda {
    ...
    struct LaunchParameters {
      ...
      LaunchParameters(dim3 gridDim, dim3 blockDim, size_t sharedMem = 0, CUstream stream = nullptr)
          : gridDim(gridDim), blockDim(blockDim), sharedMem(sharedMem), stream(stream) {}

      LaunchParameters(int gridDim, int blockDim, size_t sharedMem = 0, CUstream stream = nullptr)
          : gridDim(gridDim), blockDim(blockDim), sharedMem(sharedMem), stream(stream) {}
    };
    ...
    inline void launch_cooperative(void (*kernel)(), LaunchParameters config) {
      cuteCheck(cudaLaunchCooperativeKernel(
          (const void*)kernel, config.gridDim, config.blockDim, nullptr, config.sharedMem, config.stream));
    }
