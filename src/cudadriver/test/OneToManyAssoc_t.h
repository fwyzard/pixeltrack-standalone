...
int main() {
  ...
  cuteCheck(cuMemcpy((CUdeviceptr)v_d.get(), (CUdeviceptr)tr.data(), N * sizeof(std::array<uint16_t, 4>)));
#else
...
#ifdef __CUDACC__
  cuteCheck(cuMemcpy((CUdeviceptr)&la, (CUdeviceptr)a_d.get(), sizeof(Assoc)));
#else
...
#ifdef __CUDACC__
  cuteCheck(cuMemAlloc((CUdeviceptr*)&dc_d, sizeof(AtomicPairCounter)));
  cuteCheck(cuMemsetD8((CUdeviceptr) dc_d, 0, sizeof(AtomicPairCounter)));
  ...
  cuteCheck(cuMemcpy((CUdeviceptr)&la, (CUdeviceptr)a_d.get(), sizeof(Assoc)));
  cuteCheck(cuMemcpy((CUdeviceptr)&dc, (CUdeviceptr)dc_d, sizeof(AtomicPairCounter)));
  cuteCheck(cuMemsetD8((CUdeviceptr) dc_d, 0, sizeof(AtomicPairCounter)));
