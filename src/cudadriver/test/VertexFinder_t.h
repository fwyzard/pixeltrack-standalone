...
int main() {
  ...
  for (int nav = 30; nav < 80; nav += 20) {
    ...
    for (int i = 8; i < 20; ++i) {
...
#ifdef __CUDACC__
      cuteCheck(cuMemcpy((CUdeviceptr)LOC_WS(ntrks), (CUdeviceptr)&nt, sizeof(uint32_t)));
      cuteCheck(cuMemcpy((CUdeviceptr)LOC_WS(zt), (CUdeviceptr)ev.ztrack.data(), sizeof(float) * ev.ztrack.size()));
      cuteCheck(cuMemcpy((CUdeviceptr)LOC_WS(ezt2), (CUdeviceptr)ev.eztrack.data(), sizeof(float) * ev.eztrack.size()));
      cuteCheck(cuMemcpy((CUdeviceptr)LOC_WS(ptt2), (CUdeviceptr)ev.pttrack.data(), sizeof(float) * ev.eztrack.size()));
#else
      ...
      cuteCheck(cuMemcpy((CUdeviceptr)&nv, (CUdeviceptr)LOC_ONGPU(nvFinal), sizeof(uint32_t)));
...
#ifdef __CUDACC__
      cuteCheck(cuMemcpy((CUdeviceptr)nn, (CUdeviceptr)LOC_ONGPU(ndof), nv * sizeof(int32_t)));
      cuteCheck(cuMemcpy((CUdeviceptr)chi2, (CUdeviceptr)LOC_ONGPU(chi2), nv * sizeof(float)));
#else
      ...
      cuteCheck(cuMemcpy((CUdeviceptr)&nv, (CUdeviceptr)LOC_WS(nvIntermediate), sizeof(uint32_t)));
#else
      gridDim.x = 1;
      assert(blockIdx.x == 0);
...
#ifdef __CUDACC__
      cuteCheck(cuMemcpy((CUdeviceptr)zv, (CUdeviceptr)LOC_ONGPU(zv), nv * sizeof(float)));
      cuteCheck(cuMemcpy((CUdeviceptr)wv, (CUdeviceptr)LOC_ONGPU(wv), nv * sizeof(float)));
      cuteCheck(cuMemcpy((CUdeviceptr)chi2, (CUdeviceptr)LOC_ONGPU(chi2), nv * sizeof(float)));
      cuteCheck(cuMemcpy((CUdeviceptr)ptv2, (CUdeviceptr)LOC_ONGPU(ptv2), nv * sizeof(float)));
      cuteCheck(cuMemcpy((CUdeviceptr)nn, (CUdeviceptr)LOC_ONGPU(ndof), nv * sizeof(int32_t)));
      cuteCheck(cuMemcpy((CUdeviceptr)ind, (CUdeviceptr)LOC_ONGPU(sortInd), nv * sizeof(uint16_t)));
#endif
      for (auto j = 0U; j < nv; ++j)
        if (nn[j] > 0)
          chi2[j] /= float(nn[j]);
