...
void go() {
  ...
  cuteCheck(cuMemcpy((CUdeviceptr)v_d.get(), (CUdeviceptr)v, N * sizeof(T)));
  ...
  for (int it = 0; it < 5; ++it) {
    ...
    cuteCheck(cuMemcpy((CUdeviceptr)off_d.get(), (CUdeviceptr)offsets, 4 * (nParts + 1)));
    ...
    cuteCheck(cuMemcpy((CUdeviceptr)&h, (CUdeviceptr)h_d.get(), sizeof(Hist)));
    assert(0 == h.off[0]);
