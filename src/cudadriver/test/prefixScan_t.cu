...
int main() {
  ...
  for (int ksize = 1; ksize < 4; ++ksize) {
    ...
    cuteCheck(cuMemAlloc((CUdeviceptr *)&d_in, num_items * sizeof(uint32_t)));
    cuteCheck(cuMemAlloc((CUdeviceptr *)&d_out1, num_items * sizeof(uint32_t)));
    cuteCheck(cuMemAlloc((CUdeviceptr *)&d_out2, num_items * sizeof(uint32_t)));
    ...
    // the block counter
    int32_t *d_pc;
    cuteCheck(cuMemAlloc((CUdeviceptr *)&d_pc, sizeof(int32_t)));
    cuteCheck(cuMemsetD32((CUdeviceptr) d_pc, 0x00000000, 1));
