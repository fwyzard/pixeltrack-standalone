...
int main(void) {
  ...
  for (auto kkk = 0; kkk < 5; ++kkk) {
    ...
    cuteCheck(cuMemcpy((CUdeviceptr)d_moduleStart.get(), (CUdeviceptr)&nModules, sizeof(uint32_t)));

    cuteCheck(cuMemcpy((CUdeviceptr)d_id.get(), (CUdeviceptr)h_id.get(), size16));
    cuteCheck(cuMemcpy((CUdeviceptr)d_x.get(), (CUdeviceptr)h_x.get(), size16));
    cuteCheck(cuMemcpy((CUdeviceptr)d_y.get(), (CUdeviceptr)h_y.get(), size16));
    cuteCheck(cuMemcpy((CUdeviceptr)d_adc.get(), (CUdeviceptr)h_adc.get(), size16));
    // Launch CUDA Kernels
    ...
    std::cout << "CUDA findModules kernel launch with " << blocksPerGrid << " blocks of " << threadsPerBlock
              << " threads\n";
    cuteCheck(cuMemsetD32((CUdeviceptr) d_clusInModule.get(), 0x00000000, MaxNumModules));
    ...
    cms::cuda::launch(findClus,
                      ...
                      n);
    cuCtxSynchronize();
    cuteCheck(cuMemcpy((CUdeviceptr)&nModules, (CUdeviceptr)d_moduleStart.get(), sizeof(uint32_t)));

    uint32_t nclus[MaxNumModules], moduleId[nModules];
    cuteCheck(cuMemcpy((CUdeviceptr)&nclus, (CUdeviceptr)d_clusInModule.get(), MaxNumModules * sizeof(uint32_t)));
...
#ifdef __CUDACC__
    cuteCheck(cuMemcpy((CUdeviceptr)h_id.get(), (CUdeviceptr)d_id.get(), size16));
    cuteCheck(cuMemcpy((CUdeviceptr)h_clus.get(), (CUdeviceptr)d_clus.get(), size32));
    cuteCheck(cuMemcpy((CUdeviceptr)&nclus, (CUdeviceptr)d_clusInModule.get(), MaxNumModules * sizeof(uint32_t)));
    cuteCheck(cuMemcpy((CUdeviceptr)&moduleId, (CUdeviceptr)d_moduleId.get(), nModules * sizeof(uint32_t)));
#endif
