...
CAHitNtupletGeneratorOnGPU::CAHitNtupletGeneratorOnGPU(edm::ProductRegistry& reg)
  ...
  if (m_params.onGPU_) {
    cuteCheck(cuMemAlloc((CUdeviceptr*)&m_counters, sizeof(Counters)));
    cuteCheck(cuMemsetD8((CUdeviceptr) m_counters, 0x00, sizeof(Counters)));
  } else {
...
CAHitNtupletGeneratorOnGPU::~CAHitNtupletGeneratorOnGPU() {
  if (m_params.onGPU_) {
    ...
    cuMemFree((CUdeviceptr)m_counters);
  } else {
    if (m_params.doStats_) {
