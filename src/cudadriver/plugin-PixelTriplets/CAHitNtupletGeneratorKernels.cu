...
template <>
void CAHitNtupletGeneratorKernelsGPU::fillHitDetIndices(HitsView const *hv, TkSoA *tracks_d, CUstream stream) {
  auto blockSize = 128;
  auto numberOfBlocks = (HitContainer::capacity() + blockSize - 1) / blockSize;

  kernel_fillHitDetIndices<<<numberOfBlocks, blockSize, 0, stream>>>(
...
template <>
void CAHitNtupletGeneratorKernelsGPU::launchKernels(HitsOnCPU const &hh, TkSoA *tracks_d, CUstream stream) {
  ...
  if (nhits > 1 && m_params.earlyFishbone_) {
    ...
    dim3 blks(1, numberOfBlocks, 1);
    dim3 thrs(stride, blockSize, 1);
    gpuPixelDoublets::fishbone<<<blks, thrs, 0, stream>>>(
        hh.view(), device_theCells_.get(), device_nCells_, device_isOuterHitOfCell_.get(), nhits, false);
  }

  blockSize = 64;
  numberOfBlocks = (3 * m_params.maxNumberOfDoublets_ / 4 + blockSize - 1) / blockSize;
  kernel_find_ntuplets<<<numberOfBlocks, blockSize, 0, stream>>>(hh.view(),
                                                                     device_theCells_.get(),
                                                                     device_nCells_,
  ...
  if (m_params.doStats_)
    kernel_mark_used<<<numberOfBlocks, blockSize, 0, stream>>>(hh.view(), device_theCells_.get(), device_nCells_);
  ...
  if (m_params.doStats_) {
    ...
    kernel_checkOverflows<<<numberOfBlocks, blockSize, 0, stream>>>(tuples_d,
...
template <>
void CAHitNtupletGeneratorKernelsGPU::buildDoublets(HitsOnCPU const &hh, CUstream stream) {
  auto nhits = hh.nHits();
...
template <>
void CAHitNtupletGeneratorKernelsGPU::classifyTuples(HitsOnCPU const &hh, TkSoA *tracks_d, CUstream stream) {
  // these are pointer on GPU!
  ...
  kernel_classifyTracks<<<numberOfBlocks, blockSize, 0, stream>>>(tuples_d, tracks_d, m_params.cuts_, quality_d);

  if (m_params.lateFishbone_) {
    // apply fishbone cleaning to good tracks
    numberOfBlocks = (3 * m_params.maxNumberOfDoublets_ / 4 + blockSize - 1) / blockSize;
    kernel_fishboneCleaner<<<numberOfBlocks, blockSize, 0, stream>>>(
        device_theCells_.get(), device_nCells_, quality_d);
  }

  // remove duplicates (tracks that share a doublet)
  numberOfBlocks = (3 * m_params.maxNumberOfDoublets_ / 4 + blockSize - 1) / blockSize;
  kernel_fastDuplicateRemover<<<numberOfBlocks, blockSize, 0, stream>>>(
  ...
  if (m_params.minHitsPerNtuplet_ < 4 || m_params.doStats_) {
    ...
    kernel_countHitInTracks<<<numberOfBlocks, blockSize, 0, stream>>>(
        tuples_d, quality_d, device_hitToTuple_.get());
    cms::cuda::launchFinalize(device_hitToTuple_.get(), stream);
    kernel_fillHitInTracks<<<numberOfBlocks, blockSize, 0, stream>>>(tuples_d, quality_d, device_hitToTuple_.get());
  }
  if (m_params.minHitsPerNtuplet_ < 4) {
    // remove duplicates (tracks that share a hit)
    numberOfBlocks = (HitToTuple::capacity() + blockSize - 1) / blockSize;
    kernel_tripletCleaner<<<numberOfBlocks, blockSize, 0, stream>>>(
  ...
  if (m_params.doStats_) {
    // counters (add flag???)
    numberOfBlocks = (HitToTuple::capacity() + blockSize - 1) / blockSize;
    kernel_doStatsForHitInTracks<<<numberOfBlocks, blockSize, 0, stream>>>(device_hitToTuple_.get(), counters_);
    numberOfBlocks = (3 * CAConstants::maxNumberOfQuadruplets() / 4 + blockSize - 1) / blockSize;
    kernel_doStatsForTracks<<<numberOfBlocks, blockSize, 0, stream>>>(tuples_d, quality_d, counters_);
  }
#ifdef GPU_DEBUG
  cuCtxSynchronize();
#endif
  ...
  ++iev;
  kernel_print_found_ntuplets<<<1, 32, 0, stream>>>(
