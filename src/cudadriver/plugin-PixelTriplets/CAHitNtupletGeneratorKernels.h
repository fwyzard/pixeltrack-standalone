...
public:
  ...
  void launchKernels(HitsOnCPU const& hh, TkSoA* tuples_d, CUstream stream);

  void classifyTuples(HitsOnCPU const& hh, TkSoA* tuples_d, CUstream stream);

  void fillHitDetIndices(HitsView const* hv, TkSoA* tuples_d, CUstream stream);

  void buildDoublets(HitsOnCPU const& hh, CUstream stream);
  void allocateOnGPU(CUstream stream);
  void cleanup(CUstream stream);
