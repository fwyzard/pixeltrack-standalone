...
namespace pixelgpudetails {
  ...
  SiPixelRawToClusterGPUKernel::WordFedAppender::WordFedAppender() {
    word_ = cms::cuda::make_host_noncached_unique<unsigned int[]>(MAX_FED_WORDS, cuMemHostAllocWriteCombined);
    fedId_ = cms::cuda::make_host_noncached_unique<unsigned char[]>(MAX_FED_WORDS, cuMemHostAllocWriteCombined);
  }
  ...
  void SiPixelRawToClusterGPUKernel::makeClustersAsync(bool isRun2,
    ...
    if (wordCounter)  // protect in case of empty event....
    {
      ...
      cuteCheck(cuMemcpyAsync(
          (CUdeviceptr)word_d.get(), (CUdeviceptr)wordFed.word(), wordCounter * sizeof(uint32_t), stream));
      cuteCheck(cuMemcpyAsync(
          (CUdeviceptr)fedId_d.get(), (CUdeviceptr)wordFed.fedId(), wordCounter * sizeof(uint8_t) / 2, stream));
...

    {
      ...
      cuteCheck(cuMemcpyAsync(
          (CUdeviceptr)&nModules_Clusters_h[0], (CUdeviceptr)clusters_d.moduleStart(), sizeof(uint32_t), stream));
      ...
      cuteCheck(cuMemcpyAsync((CUdeviceptr)&nModules_Clusters_h[1],
                              (CUdeviceptr)clusters_d.clusModuleStart() + gpuClustering::MaxNumModules,
                              sizeof(uint32_t),
                              stream));
